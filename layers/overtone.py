...
class RNN4(nn.Module):
    ...
    def forward(self, x, global_cond):
        if global_cond is not None:
            # print(global_cond.shape, 'global_cond.shape 1')
            global_cond = global_cond.unsqueeze(1).expand(-1, x.size(1), -1)
        # print(global_cond.shape, 'global_cond.shape 2')
        # print(torch.cat(filter_none([x, global_cond]), dim=2).shape,'rnn')
...
class Overtone_f0(nn.Module):
    ...
    def forward(self, x, cond, global_cond):
        ...
        #audio_conds = torch.cat(filter_none([c2, cond_wav]))
        # cond0_f0 = torch.cat((cond_f0, cond_wav), dim = 1)
        # pad_x_dim = cond.shape[1] - c2.shape[1]
        # c2_x = c2.extend(torch.zeros(cond.shape))
        # c2_x[:, :c2.shape[1], :] = c2
