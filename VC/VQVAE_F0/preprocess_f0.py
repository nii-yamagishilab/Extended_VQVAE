...
def process_file(wav_file, f0_file, out_path):
    ...
    f0 = read_binfile(f0_file, dim=1, dtype=np.float32)
    ...
    print (len(quant), audio_ref_size, frm_diff)
    if frm_diff <0:
...
def main():
    ...
    if0_path = os.path.join(data_path, 'features', 'F0_confidence/F0')
    ...
    out_path = os.path.join(data_path, 'preprocessed_data_f0c')
    ...
    with mp.Pool(2*mp.cpu_count()+1) as pool:
        res = pool.starmap_async(process_file, [(f, os.path.join(if0_path, os.path.basename(f).split('.')[0]+'.f0'), out_path) for f in files]).get()
...
def main_ch():
    data_path = sys.argv[1]
    wav_path = os.path.join(data_path, 'wavs', 'wav_22050_trimmed_normalized')
    if0_path = os.path.join(data_path, 'features', 'f0_22050_64_crepe')
    scp_path = os.path.join(data_path, 'scp')
    scp_f0 = os.path.join(scp_path, 'scp_f0')
    # gender_emo_scp = os.path.join(scp_path, 'utt_gender_emotion.csv')
    # spk_emd_scp = os.path.join(scp_path, 'spk_ebd.csv')
    out_path = os.path.join(data_path, 'preprocessed_data_f0c')
    ...
    index = []
    with mp.Pool(2*mp.cpu_count()+1) as pool:
        res = pool.starmap_async(process_file, [(os.path.join(wav_path, f) + '.wav', os.path.join(if0_path, os.path.basename(f).split('.')[0]+'.f0'), out_path) for f in files]).get()
